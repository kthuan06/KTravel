<?php 
            if(isset($_POST['find']) && $_POST['keyword'] !='' && $_POST['order'] == ''){
                ...
                <div class="content grid grid-cols-5  mt-5">
                    ...
                    <?php
                        ...
                        }elseif($_POST['order'] != '' && $_POST['keyword'] == ''){
                            if($_POST['order'] == 'low'){
                                $sql = "SELECT * FROM tbl_product ORDER BY product_price ";
                                $keyword = "Thấp đến cao";
                            }else{
                                $sql = "SELECT * FROM tbl_product ORDER BY product_price DESC";
                                $keyword = "Cao đến thấp";
                            }
                            ?>
            <p class="font-bold text-xl mt-10">Từ khóa tìm kiếm: <span class="text-red-500">"<?php echo $keyword ?>"</span></p>
                <div class="content grid grid-cols-5  mt-5">
...
?>
                            ...
                            if($_POST['order'] == 'low'){
                                $sql = "SELECT * FROM tbl_product  WHERE tbl_product.product_name LIKE '%$keyword2%' OR tbl_product.product_location LIKE '%$keyword2%' ORDER BY product_price";
                                $keyword = "Thấp đến cao";
                            }else{
                                $sql = "SELECT * FROM tbl_product   WHERE tbl_product.product_name LIKE '%$keyword2%'OR tbl_product.product_location LIKE '%$keyword2%' ORDER BY product_price DESC";
                                $keyword = "Cao đến thấp";
                            }
                            ?>
    <p class="font-bold text-xl mt-10">Từ khóa tìm kiếm: <span class="text-red-500">"<?php echo $keyword2.'"  '.'  "'.$keyword ?>"</span></p>
                <div class="content grid grid-cols-5  mt-5">
                    <?php
                        $query = mysqli_query($mysqli, $sql);
                         while($row3 = mysqli_fetch_array($query)){
